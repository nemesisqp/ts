...
const HEADERS_TO_REMOVE = ['host', 'connection', 'transfer-encoding'];
...
function ClientRequest() {
    ...
    this[$stubs] = [];
    for (let i = 0; i < STUBBED_METHODS_NAME.length; i++) {
    ...
    let requestOptions, cb, url, args;
    const isInternal = arguments[0] instanceof RequestInternalEnforce;
    let isInternalMethod, isInternalProtocol;
    if (isInternal) {
...
ClientRequest.prototype = {
    ...
    take(stream) {
        ...
        //We trigger the all the stubs that were generated before
        for (let i = 0; i < this[$stubs].length; i++) {
            let stub = this[$stubs][i];
    ...
    proxyProps(http2Stream) {
        ...
        for (let i = 0; i < keysToProxy.length; i++) {
...
class HttpRequestManager extends EventEmitter {
    ...
    init(options) {
        ...
        this.http2Debouncer = new DebounceTimers(function stopConnection(key) {
            ...
            let foundConnection = this.http2Clients[key];
            if (foundConnection) {
    ...
    request(url, options, cb) {
        let args = new RequestInternalEnforce(arguments);
        ...
        if (this._events[topic])
            ...
            const socket = this.identifyConnection(requestOptions, function onIdentify(type) {
                ...
                if (type === 'h2' && this.http2Support) {
                    let http2Client = this.http2.connect(requestOptions, options);
    ...
    makeHttpRequest(clientKey, inStream, options, cb, connectionOptions) {
        ...
        const protocol = h1op.protocol || h1op.uri.protocol;
        const requestModule = protocol === 'https:' ? this.https : this.http;
    ...
    makeHttp2Request(clientKey, inStream, http2Client, requestOptions, cb) {
        let http2Debouncer = this.http2Debouncer;
        http2Debouncer.pause(clientKey);
        let headers = _extend({}, requestOptions.headers || {});
    ...
    identifyConnection(requestOptions, cb) {
        let socket = this.connect(requestOptions, function onConnect() {
    ...
    connect(authority, options, listener) {
        ...
        let protocol = authority.protocol || options.protocol || (this.enforceProtocol !== 'detect' ? this.enforceProtocol : null) || 'http:';
